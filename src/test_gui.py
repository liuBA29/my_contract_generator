...
from src.database import fetch_customer_data, fetch_completion_of_work_by_condition, fetch_all_payment_terms, \
    fetch_payment_terms_by_id
...
def generate_contract():
    try:
        ...
        if payment_condition == 'предоплата':
            ...
            term_id = int(payment_term_var.get())
...
tk.Label(root, text="Введите ID клиента:").grid(row=0, column=0, sticky="w", padx=10, pady=5)
entry_customer_id = tk.Entry(root, width=50)
entry_customer_id.grid(row=0, column=1, padx=10, pady=5)

# Поле для выбора типа оплаты
tk.Label(root, text="Выберите тип оплаты:").grid(row=1, column=0, sticky="w", padx=10, pady=5)
...
payment_condition_combobox = ttk.Combobox(root, textvariable=payment_condition_var, values=["предоплата", "постоплата"],
                                          state="readonly")
payment_condition_combobox.grid(row=1, column=1, padx=10, pady=5)

# Поле для выбора процента предоплаты
tk.Label(root, text="Выберите процент предоплаты:").grid(row=2, column=0, sticky="w", padx=10, pady=5)
payment_term_var = tk.StringVar()
payment_term_combobox = ttk.Combobox(root, textvariable=payment_term_var, state="disabled")
payment_term_combobox.grid(row=2, column=1, padx=10, pady=5)
...
def update_payment_terms(event):
    if payment_condition_var.get() == "предоплата":
        ...
        payment_terms = fetch_all_payment_terms()
        payment_term_combobox['values'] = [str(row[0]) for row in payment_terms[1:]]
    else:
...
tk.Label(root, text="Введите номер договора:").grid(row=3, column=0, sticky="w", padx=10, pady=5)
entry_contract_number = tk.Entry(root, width=50)
entry_contract_number.grid(row=3, column=1, padx=10, pady=5)

# Поле для ввода места проведения работ
tk.Label(root, text="Введите место проведения работ:").grid(row=4, column=0, sticky="w", padx=10, pady=5)
entry_location = tk.Entry(root, width=50)
entry_location.grid(row=4, column=1, padx=10, pady=5)

# Поле для ввода даты
tk.Label(root, text="Введите дату (дд.мм.гггг):").grid(row=5, column=0, sticky="w", padx=10, pady=5)
entry_doc_date = tk.Entry(root, width=50)
entry_doc_date.grid(row=5, column=1, padx=10, pady=5)

# Поле для ввода работ
tk.Label(root, text="Введите работы:").grid(row=6, column=0, sticky="w", padx=10, pady=5)
entry_work = tk.Entry(root, width=50)
entry_work.grid(row=6, column=1, padx=10, pady=5)

# Список для отображения введенных работ
work_listbox = tk.Listbox(root, width=50, height=5)
work_listbox.grid(row=7, column=1, padx=10, pady=5)
...
add_work_button.grid(row=6, column=2, padx=10, pady=5)

# Поле для ввода стоимости работ
tk.Label(root, text="Введите стоимость работ (в рублях):").grid(row=8, column=0, sticky="w", padx=10, pady=5)
entry_total_cost = tk.Entry(root, width=50)
entry_total_cost.grid(row=8, column=1, padx=10, pady=5)

# Кнопка для генерации договора
generate_button = tk.Button(root, text="Сгенерировать договор", command=generate_contract)
generate_button.grid(row=9, column=1, padx=10, pady=20)
